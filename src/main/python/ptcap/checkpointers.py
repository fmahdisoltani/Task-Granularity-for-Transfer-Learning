...
class Checkpointer(object):
    ...
    def load_model(self, model, optimizer, folder=None, filename=None,
                   load_encoder_only=False, load_decoder_only=False):
        ...
        if pretrained_path is None:
            ...
            if load_encoder_only:
                state_dict = {key.replace('module.encoder.', ''): value for
                              key, value in checkpoint['model'].items()
                              if 'module.encoder' in key}
            elif load_decoder_only:
                state_dict = {key.replace('module.decoder.', ''): value for
                              key, value in checkpoint['model'].items()
                              if 'module.decoder' in key}
