...
def create_subset_json_balanced2(path, target_classes, num_samples=None):
    """
    This function is the result of "challenge accepted"
    to write the above function in only one line
    """

    return [item for sublist in
            [[x for x in open_json(path) if x == t][:num_samples]
             for t in [sample for sample in json if
                       sample['template'] in target_classes]] for item in
            sublist]
